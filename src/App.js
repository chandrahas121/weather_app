...
const WeatherApp = () => {
  ...
  const getWeather = () => {
    if (city.length === 0) {
      ...
      const url = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=bf266ec499f78c9c77c
eab370f7faa8b&units=metric`;
      setCity("");
      fetch(url)
        .then((resp) => resp.json())
        .then((data) => {
...
Icon"
        ...
        .catch(() => {
          fetch(`https://www.stands4.com/services/v2/grammar.php?uid=12187&tokenid=y
qFWz075iEFkvHgS&text=${city}&format=json`)
            .then((resp) => resp.json())
            .then((data) => {
